---  
title: "Lab 3: Beyond the Basics"  
description: Execute ROS applications more efficiently using launch files, and learn how to affect the behaviour of nodes during run-time using parameters. Learn about the LiDAR sensor, the data that it generates, and see the benefits of this for tools like "SLAM".
---
...
### Exercice 1: Creating a Launch File {#ex1}
...
1. We're going to launch our robot into *another* new simulated environment now, which we'll be creating a map of using SLAM! To launch the simulation enter the following command in **TERMINAL 1**:
    ...
    ```bash
    ros2 launch simulations nav_world.launch.py
    ```
...
1. Now launch SLAM to start building a map of this environment. In **TERMINAL 2**, launch SLAM as follows:
    ...
    ```bash
    ros2 launch tb3_tools slam.launch.py environment:=sim
    ```
...
1.  **Create the file**: In your `scripts` folder, create a file named `obstacle_avoidance.py` and make it executable.
2.  **The Parameters**: 
    * Declare a parameter named `safety_distance` with a default value of `0.5` (meters).
3.  **The Logic**: 
    * Subscribe to the `/scan` topic.
    * Monitor the minimum distance in a small arc (e.g., the front 10 degrees) of the `ranges` array.
    * If the distance to an object is **greater** than `safety_distance`, publish a linear velocity of `0.1 m/s`.
    * If an object is detected **closer** than `safety_distance`, stop the robot.
4.  **Test with Parameters**: Once the node is running, use `ros2 param set` to change the `safety_distance` while the robot is moving and observe how it reacts.
...
2.  **Compress your package**: 
    ```bash
    zip -r part3_submission.zip part3_beyond_basics/
    ```
...
* **`launch/`**: 
    * `pubsub.launch.py` (Launching both publisher and subscriber).
* **`scripts/`**: 
    1. `param_circle.py` (The parameter-driven circle node).
    2. `lidar_subscriber.py` (The LiDAR data processing node).
    3. `obstacle_avoidance.py` (**The Final Challenge**).
* **`CMakeLists.txt`**: Must include the `install(DIRECTORY launch ...)` and all script executables.
* **The Goal Coordinates**: Include a simple text file or comment in your code with the **Zone A, B, C, and D coordinates** you found during the SLAM exercise.
